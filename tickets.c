...
void sync_threads() {
    pthread_barrier_wait(&barrierSyn);
...
void *cashier(void *arg)
{
	...
	printf(C_CASHIER "cashier:     waiting for all reservations to finish" C_NORMAL "\n");
        ...
        sem_post(&pokladnikSem);
        sem_wait(&vlastnikSem);
...
int main(int argc, char *argv[])
{
    ...
    sem_init(&vlastnikSem,0,SEM_SYNC_INIT_VALUE);

    //nastavení bariéry pro synchronní start všech vláken
    pthread_barrier_init(&barrierSyn,NULL,customers + 2);
	...
	for (c = 0; c < customers; ++c) {
        ...
        //vlákno zákazníka počká než se ukončí předchozí vlákno - nespouští se synchronně
    ...
    pthread_barrier_destroy(&barrierSyn);
